...
baseline_rec <- recipe(price_log10 ~., data = airbnb_train) %>%
...
baseline_nonpara_rec <- recipe(price_log10 ~., data = airbnb_train) %>%
  update_role(id, new_role = "id") %>%
  step_date(all_date_predictors(), keep_original_cols = FALSE) %>%
  step_impute_mode(all_nominal_predictors()) %>%
  step_impute_median(all_numeric_predictors()) %>%
...
### variation 1 (parametric)
fe_rec <- recipe(price_log10 ~., data = airbnb_train) %>%
  update_role(id, new_role = "id") %>%
  step_date(all_date_predictors(), keep_original_cols = FALSE) %>%
  step_impute_median(all_numeric_predictors()) %>%
  step_impute_mode(all_nominal_predictors()) %>%
...
### variation 2 (non-parametric)
fe_nonpara_rec <- recipe(price_log10 ~., data = airbnb_train) %>%
  update_role(id, new_role = "id") %>%
  step_date(all_date_predictors(), keep_original_cols = FALSE) %>%
  step_impute_median(all_numeric_predictors()) %>%
  step_impute_mode(all_nominal_predictors()) %>%
...
fe_tree_rec <- recipe(price_log10 ~., data = airbnb_train) %>%
  update_role(id, new_role = "id") %>%
  step_date(all_date_predictors(), keep_original_cols = FALSE) %>%
  step_impute_median(all_numeric_predictors()) %>%
  step_impute_mode(all_nominal_predictors()) %>%
...
save(fe_tree_rec, file = here("recipes/round_1/fe_tree_rec.rda"))
